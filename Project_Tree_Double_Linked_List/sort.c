...
void SwapNode(scoresSt *Entry1, scoresSt *Entry2){
    ...
    strcpy((char*)Temp.Name, (char*)Entry1->Name);
    Temp.ComputerScience = Entry1->ComputerScience;
    /* Put date of Entry2 in Entry1 */
    strcpy((char*)Entry1->Name, (char*)Entry2->Name);
    Entry1->ComputerScience = Entry2->ComputerScience;
    /* Put date of Temp in Entry2 */
    strcpy((char*)Entry2->Name, (char*)Temp.Name);
    Entry2->ComputerScience = Temp.ComputerScience;
}//SwapNode
s8 Partition(scoresSt **Scores, s8 Low, s8 High){
    s8 Pivot;
    Pivot = ((*Scores)+High)->ComputerScience;
    s8 i = Low-1;

    for(s8 j = Low; j <= High-1; j++){
        if(((*Scores)+j)->ComputerScience >= Pivot){
        i++;
        SwapNode(((*Scores)+j), ((*Scores)+i));
        }//if
    }//for
    SwapNode(((*Scores)+i+1), ((*Scores)+High));
    return(i+1);
}//Partition
void QuickSort(scoresSt **Scores, s8 Low, s8 High){
    ...
    if (Low < High)
    {
        P = Partition(Scores, Low, High);
        QuickSort(Scores, Low, P-1);
        QuickSort(Scores, P+1, High);
    }//if
}//QuickSort
